...
if __name__ == "__main__":
    ...
    while (current_y < h):
        ...
        while (current_x < w):
            ...
            output_name_base = './out/' + file_name.split('/')[-1][:-4] + '_' + name
            output_name_png = output_name_base + '.png'
            output_name_pdf = output_name_base + '.pdf'
            image_croped_margine.save(output_name_png, 'PNG', quality=100)
            import os
            os.system(f"convert '{output_name_png}' -page A4 '{output_name_pdf}'")
            os.system(f"rm '{output_name_png}'")
            generated_files.append(output_name_pdf)
            current_x += CROP_WIDTH
            x_count += 1
        current_y += CROP_HIGHT
        y_count += 1

    generated_files_string = ""
    for f in generated_files:
        generated_files_string += '\"' + f + '\"'
        generated_files_string += " "
    os.system(f"convert {generated_files_string} './out/{file_name.split('/')[-1][:-4]}.pdf'")
    for f in generated_files:
        os.system(f"rm '{f}'")
